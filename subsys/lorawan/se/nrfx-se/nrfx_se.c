...
{
	ssize_t len = hwinfo_get_device_id(buf, 8);
	if (len < 0) {
		return len;
	}
...
SecureElementStatus_t SecureElementSetKey(KeyIdentifier_t keyID, uint8_t *key)
{
	...
	if (need_encrypt) {
		...
		mbedtls_internal_aes_encrypt(&ctx, key, se_key.KeyValue);
		mbedtls_aes_free(&ctx);
	} else {
		memcpy(se_key.KeyValue, key, 16);
	}
...
static int nrfx_se_get_decrypted_key(KeyIdentifier_t id, uint8_t *out)
{
	...
	if (!need_decrypt) {
		...
		mbedtls_internal_aes_decrypt(&ctx, se_key.KeyValue, out);
		mbedtls_aes_free(&ctx);
		return SECURE_ELEMENT_SUCCESS;
	}
}
...
SecureElementComputeAesCmac(uint8_t *micBxBuffer, uint8_t *buffer,
			    uint16_t size,
...
#if(USE_LRWAN_1_1_X_CRYPTO == 1) /* TODO: ? */
